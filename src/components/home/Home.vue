<template>
  <div>
    ...
    <v-navigation-drawer temporary v-model="drawer" app>
      ...
      >
        <v-list-item link>
          <v-list-item-title v-text="hashtag.title"/>
          <v-list-item-icon>
    ...
    <v-container>    
      <v-row align="start">
        <v-col v-for="(hashtag, index) in hashtags" :key="index" ref="hashtags">
          ...
          >
            <v-toolbar class="white--text" color="#1ca0cc" flat dark>
              ...
              <v-spacer/>
              <v-icon dark v-text="'mdi-refresh'"/>
              <v-icon dark v-text="'mdi-minus'"/>
            </v-toolbar>
            <div class="scrollable" v-if="hashtag.tweets">
              <div v-if="hashtag.tweets.length == 0" class="text-center mt-5">
                <v-list-item>
                  ...
                  <v-btn color="#1ca0cc" dark rounded v-text="'Try Again'"/>
                </v-list-item>
              </div>
              <v-list 
                v-else
        ...
        <v-dialog v-model="dialog" persistent max-width="600px">
          ...
          <v-card>
            <v-card-title>
              <span class="headline">Mark new #HashTag</span>
            </v-card-title>
            <v-card-text>
              <v-container>
                <v-row>
